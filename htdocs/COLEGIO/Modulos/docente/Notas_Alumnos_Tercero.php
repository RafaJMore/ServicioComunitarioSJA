	 	...
	 	<center>
	 		...
	 		<legend><b>Registrando Notas:</b></legend>
	 			...
	 			<select name="materia_cod_materia">
	 				<option> </option>
	 					<?php  include '../../server/conexion.php';
							$sql="SELECT cod_materia, nombre_materia, docente_ci_docente FROM materia WHERE docente_ci_docente='".$_SESSION['usuario']['cedula']."'";	
							$result = mysqli_query($conn, $sql);
							while($fila=mysqli_fetch_assoc($result)){	
			...
			$sql = "SELECT ci_alum, primer_nom_alum,  primer_ape_alum  FROM alumno WHERE año_idaño=3";
			$result = mysqli_query($conn, $sql);
